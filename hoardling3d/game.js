...
(function () {
  ...
  var R3D = {
    ...
    // its art matures.
    on: !/[?&]r3d=0/.test(location.search),   // 3D IS THIS PROJECT (?r3d=0 = compare against the painted path)
    ...
    boot: function (game) {
      ...
      import('./proto3d/three.module.js').then(function (T) {
        ...
        scene.background = new T.Color(0x1b2438);
        scene.fog = new T.Fog(0x233047, 820, 1750);    // cool haze eats the horizon
        self.scene = scene;
        self.cam = new T.PerspectiveCamera(44, 1, 1, 2600);
        var key = new T.DirectionalLight(0xffe6c0, 4.2);
        ...
        scene.add(key);
        // cool sky / warm bounce: the temperature SPLIT is the depth cue.
        // Keep it MODEST — a strong fill erases the key's shading and the
        // whole scene goes bleached and flat (measured that the hard way).
    ...
    resize: function (game) {
      ...
      // FREE OF THE 2D LAYOUT. The shipping game frames the world flat so the
      // painted HUD lines up; this project answers only to how it looks, so
      // the camera drops and comes in for a proper three-quarter view.
      this._camBase = { x: 0, y: 620, z: 880 };
      this.cam.position.set(this._camBase.x, this._camBase.y, this._camBase.z);
      this.cam.lookAt(0, 40, -120);
    ...
    buildWorld: function (game) {
      ...
      // GROUND — a single flat-coloured plane is the loudest "cheap" tell in
      // low-poly: real ground has patch variation and a little relief so the
      // key light finds facets. Segmented plane + per-vertex colour + gentle
      // displacement, all baked once at build time (zero per-frame cost).
      var floorGeo = new T.PlaneGeometry(4200, 4200, 46, 46);
      var fpos = floorGeo.attributes.position, fcol = [];
      for (var vi = 0; vi < fpos.count; vi++) {
        var vx2 = fpos.getX(vi), vy2 = fpos.getY(vi);
        var d2c = Math.sqrt(vx2 * vx2 + vy2 * vy2);
        // relief rises AWAY from the play area so the road stays flat/readable
        var relief = Math.max(0, (d2c - 420) / 900);
        var bump = (Math.sin(vx2 * 0.013) * Math.cos(vy2 * 0.011) +
                    Math.sin(vx2 * 0.031 + 1.7) * 0.4) * 16 * relief;
        fpos.setZ(vi, bump);
